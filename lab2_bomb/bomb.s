...
0000000000400ee0 <phase_1>:
  ; phase_1 has one parameters in register "%rdi".

  ; "sub %rsp" reserve stack frame for local variable. 
  400ee0:	48 83 ec 08          	sub    $0x8,%rsp
  
  ; mov $0x402400 to register "%esi" 
  ; as the second parameter of function "string_not_equal".
  400ee4:	be 00 24 40 00       	mov    $0x402400,%esi
  ; "strings_not_equal" return 1(to %rax) if those two string is not equal.
  400ee9:	e8 4a 04 00 00       	callq  401338 <strings_not_equal>

  ; "test" instruction set condition codes according to S1 & S2.
  ; Register "%eax" stores return value of function strings_not_equal.
  400eee:	85 c0                	test   %eax,%eax

  ; "je" instruction means: jump if equal or zero(condition code).
  ; Here, jump if %eax is zero.
...
0000000000400efc <phase_2>:
  ; %rbp and %rbx are callee-save register.
  400efc:	55                   	push   %rbp
  400efd:	53                   	push   %rbx

  ; Read six number from the string(the address of witch is in %rdi) into stack.
  400efe:	48 83 ec 28          	sub    $0x28,%rsp

  ; Save current stack pointer in %rsi, as para2 to read_six_number
  400f02:	48 89 e6             	mov    %rsp,%rsi
  400f05:	e8 52 05 00 00       	callq  40145c <read_six_numbers>

  ; "cmp" set the condition codes to S1 - S2.
  ; Compare the first number with 1;
  ...
  ; set %rbx as the index pointer, %rbp as the boundary of this loop
...
0000000000400f43 <phase_3>:
  ...
  ; "%d %d" in $0x4025cf
  ; Addr of input string in %rdi
  400f51:	be cf 25 40 00       	mov    $0x4025cf,%esi
  400f56:	b8 00 00 00 00       	mov    $0x0,%eax
  400f5b:	e8 90 fc ff ff       	callq  400bf0 <__isoc99_sscanf@plt>
  ; Ensure more than 1 number read in. 
  400f60:	83 f8 01             	cmp    $0x1,%eax
  400f63:	7f 05                	jg     400f6a <phase_3+0x27>
  400f65:	e8 d0 04 00 00       	callq  40143a <explode_bomb>
  ; The first number can't be above 7 (==7 don't jump)
  ; ja -> jump above, for unsigned, and `cmpl` is for unsigned too 
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp)
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a>
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax
  ; Jump to *(0x402470 + 8 * $rax)
  ; `jmpq` means jump to a absolute addr
...
000000000040100c <phase_4>:
  ...
  ; `rdx` save the addr of 1st number; `rcx` the 2nd number
  ...
  ; Thus the first number must be <= 0xe;
  40102e:	83 7c 24 08 0e       	cmpl   $0xe,0x8(%rsp)
  401033:	76 05                	jbe    40103a <phase_4+0x2e>
  401035:	e8 00 04 00 00       	callq  40143a <explode_bomb>
  ; Set parameters; "1st number" in $rdi; 0 in $rsi; 0xe in $rdx.
  40103a:	ba 0e 00 00 00       	mov    $0xe,%edx
  40103f:	be 00 00 00 00       	mov    $0x0,%esi
  401044:	8b 7c 24 08          	mov    0x8(%rsp),%edi
  401048:	e8 81 ff ff ff       	callq  400fce <func4>
  ; If %eax == 0, than ZF = 0, thus jne don't token.
  ; So the return value of func4 should be 0.
  40104d:	85 c0                	test   %eax,%eax
  40104f:	75 07                	jne    401058 <phase_4+0x4c>
  ; The 2nd number must be 0;
...
000000000040145c <read_six_numbers>:
  ...
  ; Address of the string containing the six numbers stores 
  ; in the first parameter register %rdi;

  40145c:	48 83 ec 18          	sub    $0x18,%rsp

  ; Set parameter in %rdx(the address in stack to store the first number).
  401460:	48 89 f2             	mov    %rsi,%rdx

  ; Set parameter in %rcx(address to store 2nd number).
  401463:	48 8d 4e 04          	lea    0x4(%rsi),%rcx

  ; Set parameter in stack(%rsp + 0x8) (6th number).
  401467:	48 8d 46 14          	lea    0x14(%rsi),%rax
  40146b:	48 89 44 24 08       	mov    %rax,0x8(%rsp)

  ; Set parameter in stack(%rsp) (5th number).
  401470:	48 8d 46 10          	lea    0x10(%rsi),%rax
  401474:	48 89 04 24          	mov    %rax,(%rsp)

  ; Set parameter in %r9 (4th number).
  401478:	4c 8d 4e 0c          	lea    0xc(%rsi),%r9

  ; Set parameter in %r8 (3rd number).
  40147c:	4c 8d 46 08          	lea    0x8(%rsi),%r8

  ; Set parameter int %rsi (address of the format string: "%d %d %d %d %d %d").
  ...
  ; The return value of this function is amount of number read in.
  40148a:	e8 61 f7 ff ff       	callq  400bf0 <__isoc99_sscanf@plt>

  ; Exam amount of those read numbers; Must be greater than 5. 
